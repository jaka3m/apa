...
const vmessUUID = process.env.UUID || atob('ZjI4MmI4NzgtODcxMS00NWExLThjNjktNTU2NDE3MjEyM2Mx');
        ...
        write(chunk) {
            (async () => {
                for (let index = 0; index < chunk.byteLength;) {
                    ...
                    const udpChunk = new Uint8Array(chunk.slice(index + 2, index + 2 + udpPacketLength));
                    ...
                    try {
                        ...
                        if (webSocket.readyState === 1) { // OPEN
                            ...
                            if (isHeaderSent) {
                                ...
                                if (responseHeader) {
                                    webSocket.send(Buffer.concat([Buffer.from(responseHeader), Buffer.from(udpSizeBuffer), Buffer.from(dnsQueryResult)]));
                                } else {
                                    webSocket.send(Buffer.concat([Buffer.from(udpSizeBuffer), Buffer.from(dnsQueryResult)]));
                                }
                                isHeaderSent = true;
...
function handleTCPOutbound(addressRemote, portRemote, rawClientData, webSocket, responseHeader, log, pxip) {
    let remoteSocket;
    ...
    function connectAndWrite(address, port) {
        const socket = net.connect(port, address, () => {
            ...
            socket.write(Buffer.from(rawClientData));
        });

        socket.on('data', (chunk) => {
            if (responseHeader && !headerSent) {
                webSocket.send(Buffer.concat([Buffer.from(responseHeader), chunk]));
                headerSent = true;
            } else {
        ...
        socket.on('error', (err) => {
            log(`Remote socket error (${address}:${port})`, err.message);
            if (!headerSent && pxip) {
                log('Retrying with proxy IP...');
                const parts = pxip.split(':');
                connectAndWrite(parts[0], parseInt(parts[1]) || 443);
            } else {
                webSocket.close();
            }
        });
...
<body class="min-h-screen py-4 md:py-8 px-3 md:px-6 relative transition-colors duration-300">
    ...
    <script>
        ...
        const uuid = atob('{{UUID_BASE64}}');
        const host = window.location.host;
        const proxyListUrl = atob('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL2pha2ExbS9ib3Rhay9yZWZzL2hlYWRzL21haW4vY2VrL3Byb3h5TGlzdC50eHQ=');
        const OBFS_PATH = atob('L0ZyZWUtVlBOLUNGLUdlby1Qcm9qZWN0Lw==');
        ...
        const SS_LBL = '[Shadowsocks]';
        const CHECK_API_URL = atob('aHR0cHM6Ly9jaGVjay5ncGozLndlYi5pZC9jaGVjaw==');
        const countryNameFormatter = new Intl.DisplayNames(['en'], { type: 'region' });
        function getCountryFullName(countryCode) {
            if (!countryCode) return 'Unknown';
            try { return countryNameFormatter.of(countryCode.toUpperCase()) || countryCode; } catch (error) { return countryCode; }
        }
        let allProxies = [];
        let filteredProxies = [];
        let currentPage = 1;
        ...
        async function checkProxyStatus(ip, port) {
            ...
            try {
                ...
                const result = { status: data.status || 'UNKNOWN', delay: data.delay || 'N/A', speed: data.speed_est || 'N/A' };
        ...
        async function fetchProxies() {
            try {
                ...
                allProxies = text.trim().split('\\n').map(line => {
                    ...
                    return { ip, port, country: getCountryFullName(country), isp, countryCode: country, status: null };
        ...
        async function checkAllProxyStatuses() {
            ...
            for (let i = 0; i < filteredProxies.length; i += batchSize) {
                ...
                await Promise.all(batch.map(async (proxy, idx) => {
                    ...
                    updateProxyRowInTable(i + idx, proxy);
                }));
            }
        }
        function updateProxyRowInTable(proxyIndex, proxy) {
            const start = (currentPage - 1) * itemsPerPage;
            if (proxyIndex >= start && proxyIndex < start + itemsPerPage) {
                const rows = document.getElementById('proxyTableBody').getElementsByTagName('tr');
                const row = rows[proxyIndex - start];
                if (row) row.querySelector('.status-cell').innerHTML = getStatusHtml(proxy);
            }
        }
        function generateVmess(proxy) {
            const path = OBFS_PATH + proxy.ip + "=" + proxy.port;
            const vmessObj = { v: "2", ps: VMS_LBL + " " + proxy.country, add: host, port: 443, id: uuid, aid: "0", scy: "zero", net: "ws", type: "none", host: host, path: path, tls: "tls", sni: host };
        ...
        function generateShadowsocks(proxy) {
            const encodedAuth = btoa(\`none:\${uuid}\`);
            const path = encodeURIComponent(OBFS_PATH + proxy.ip + "=" + proxy.port);
        ...
        function copyToClipboard(text, btn) {
            navigator.clipboard.writeText(text).then(() => {
                const original = btn.innerHTML; btn.innerHTML = '<i class="fas fa-check"></i> Copied';
        ...
        function getStatusHtml(proxy) {
            ...
            if (proxy.status === 'ACTIVE') return \`<div class="status-badge status-active tooltip" data-tooltip="Delay: \${proxy.delay} | Speed: \${proxy.speed}"><i class="fas fa-check-circle animate-pulse text-green-500"></i><span class="animate-pulse text-green-500 font-semibold">ACTIVE</span></div>\`;
        ...
        function renderTable() {
            ...
            tbody.innerHTML = paged.map((proxy, idx) => {
                const id = start + idx;
                return \`
                        ...
                        <td class="py-4 px-4 text-right relative dropdown-container">
                            ...
                            <div id="drop-\${id}" class="dropdown-menu">
                                <div class="grid grid-cols-2 gap-2">
                                    <button onclick="copyToClipboard('\${generateVless(proxy)}', this)" class="bg-indigo-600 hover:bg-indigo-700 p-2 rounded-md text-[10px] font-bold text-white flex flex-col items-center gap-1"><i class="fas fa-link"></i> VLESS</button>
                                    <button onclick="copyToClipboard('\${generateTrojan(proxy)}', this)" class="bg-purple-600 hover:bg-purple-700 p-2 rounded-md text-[10px] font-bold text-white flex flex-col items-center gap-1"><i class="fas fa-shield-halved"></i> TROJAN</button>
                                    <button onclick="copyToClipboard('\${generateShadowsocks(proxy)}', this)" class="bg-cyan-600 hover:bg-cyan-700 p-2 rounded-md text-[10px] font-bold text-white flex flex-col items-center gap-1"><i class="fas fa-lock"></i> Shadowsocks</button>
                                    <button onclick="copyToClipboard('\${generateVmess(proxy)}', this)" class="bg-emerald-600 hover:bg-emerald-700 p-2 rounded-md text-[10px] font-bold text-white flex flex-col items-center gap-1"><i class="fas fa-bolt"></i> VMESS</button>
                                </div>
                            </div>
                         </td>
                     </tr>\`;
...
app.get('/', (req, res) => {
    const uuid_base64 = btoa(vmessUUID);
    res.send(UI_HTML.replace('{{UUID_BASE64}}', uuid_base64));
});
...
wss.on('connection', async (ws, req) => {
    ...
    const log = (info, event) => console.log(`[${req.socket.remoteAddress}] ${info}`, event || "");
    ...
    let pxip = null;
    if (match) pxip = match[1].replace(/[=-]/, ':');
    else if (oldMatch) pxip = oldMatch[1].replace(/[=-]/, ':');
    ...
    ws.on('message', async (data) => {
        const bufferChunk = new Uint8Array(data);
        ...
        if (isFirstPacket) {
            ...
            try {
                ...
                if (isDNS) {
                    ...
                    handleTCPOutbound(protocolHeader.addressRemote, protocolHeader.portRemote, protocolHeader.rawClientData, ws, protocolHeader.version, log, pxip);
                }
            } catch (err) {
                log("Header parse error", err.message);
                ws.close();
            }
        } else {
            if (isDNS && udpStreamWrite) {
                udpStreamWrite(bufferChunk);
            }
        }
    });
    ...
    if (earlyDataHeader) {
        ...
        if (earlyData) {
            ws.emit('message', Buffer.from(earlyData));
        }
    }
});
